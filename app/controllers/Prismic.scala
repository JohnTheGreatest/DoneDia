...
object Prismic extends Controller {
  ...
  // -- Helper: Check if the slug is valid and redirect to the most recent version id needed
  def checkSlugAndCategory(document: Option[Document], slug: String, category: String)(callback: Either[String, Document] => Result)(implicit r: Prismic.Request[_]) = {
    document.collect {
      case document if (document.slug == slug && document.getText("blog.category").get == BlogCategories.get(category)) => callback(Right(document))
      case document if (document.slugs.contains(slug) && document.getText("blog.category").get == BlogCategories.get(category)) => callback(Left(document.slug))
    }.getOrElse {
      Application.PageNotFound
    }
  }
